...
cursor = connect(s3_staging_dir="s3://${athena_s3_bucket}/results/",
...
FROM (${athena_db}.${ticket_details_table_name} CROSS JOIN UNNEST(cases) AS t(item))) CROSS JOIN UNNEST(comm) as t(detail)) as at
inner join
(SELECT item FROM (${athena_db}.${tickets_table_name} CROSS JOIN UNNEST(cases) t (item))) as ac on ac.item.caseid = at.detail.caseid
{where_clause}
